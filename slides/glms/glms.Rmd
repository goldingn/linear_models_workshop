...
There are other information criteria, e.g. the 'Bayesian' Information Criterion
$$
BIC = -2 \times ln(\hat{L}) + ln(n) \times k
$$

People like to argue about which of these are best.
...
An alternative is to calculate the likelihood on some data that weren't used to fit the model.

This takes longer (you need to fit the model lots of times) and won't work well if you have very little data.
